document.addEventListener("DOMContentLoaded", function () {
  ...
  if (document.getElementById("following")) {
    document
      ...
      .addEventListener("click", () => load_posts("/followed", 1));
  } else {
    document
      .getElementById("newPost")
      .addEventListener("click", () => force_login());
  }
  load_posts("", 1);
});
...
function update_like(post) {
  fetch(`/post/${post.id}/update_like`)
    ...
    .then((response) => {
      if (response.liked) {
        document.getElementById(`like-icon-${post.id}`).className =
          "icon-heart col-auto";
      } else {
        document.getElementById(`like-icon-${post.id}`).className =
          "icon-heart-empty col-auto";
      }
      document.getElementById(`likes-amount-${post.id}`).innerHTML =
        response.newAmount;
    });
}
